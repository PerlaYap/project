...
$terminate = $this->db->query("SELECT c.CenterNo, mn.FirstName, mn.MiddleName, mn.LastName, b.BranchName, bc.CaritasBranch_ControlNo, cm.CaritasCenters_ControlNo, m.ControlNo, Date(ms.DateUpdated) AS Date, m.MemberID
			...
			WHERE b.ControlNo = bc.CaritasBranch_ControlNo AND bc.CaritasCenters_ControlNo = c.ControlNo AND c.ControlNo = cm.CaritasCenters_ControlNo AND cm.Members_ControlNo = m.ControlNo AND m.ControlNo = mn.ControlNo AND mn.ControlNo = ms.ControlNo AND ms.Status = 'Terminated Voluntarily' Order by C.CenterNo");
